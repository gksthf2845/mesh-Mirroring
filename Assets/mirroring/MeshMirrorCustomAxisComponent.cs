...
public class MeshMirrorCustomAxisComponent : MonoBehaviour
{
    // 미러링할 축 선택
    ...
    public void UpdateMirroredMesh()
    {
        ...
        Vector3[] mirroredNormals = new Vector3[originalNormals.Length];
            ...
            Vector3 axis = customAxis.normalized;
            ...
            for (int i = 0; i < originalVertices.Length; i++)
            {
                ...
                switch (mirrorAxis)
                {
                    ...
                    case MirrorAxis.Custom:
                        ...
                        // 반사 공식 적용: v' = v - 2 * (v·n) * n
                        float VertexDot = Vector3.Dot(originalVertices[i], axis);
                        mirroredVertices[i] = vertex - 2 * VertexDot * axis;

                        // 노멀도 같은 방식으로 미러링
                        float normalDot = Vector3.Dot(originalNormals[i], axis);
                        mirroredNormals[i] = normal - 2 * normalDot * axis;
                        break;

                }
            //커스텀 축 일 경우에면 따로 버텍스 연산
            if (mirrorAxis != MirrorAxis.Custom)
            {
    ...
    private void UpdateMirroredPosition()
    {
        ...
        switch (mirrorAxis)
        {
            ...
            case MirrorAxis.Custom:
                ...
                // 입사벡터에 위치벡터 입력
                float dot = Vector3.Dot(mirroredPosition, axis);

                mirroredPosition = mirroredPosition - 2 * dot * axis;
                break;
        }
    ...
    private void UpdateMirroredRotation()
    {
        ...
        switch (mirrorAxis)
        {
            ...
            case MirrorAxis.Custom:
                ...
                // 축을 기준으로 180도 회전 
                Quaternion axisRotation = Quaternion.AngleAxis(180f, axis);

                // 미러링 회전 = 축 회전 * 원본 회전에 -w 곱하기 추가
                mirroredRotation = axisRotation * 
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 direction = customAxis.normalized;
        int gizmoSize = 5;
        // 축 방향 선 그리기 (양방향)
        Gizmos.DrawLine(start - direction * gizmoSize, start + direction * gizmoSize);
    }
}